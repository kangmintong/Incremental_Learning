...
# import numpy as np
# import torch
# import network.resnet_ours as resnet_ours
...
# import math
# from eval import eval_ours_ncm
#
# cur_features = []
# ref_features = []
# old_scores = []
...
#     parser.add_argument('-incre_num_classes',type=int,default=1)
...
#     torch.save(index_train,'./../checkpoints/ours/index_train')
#     lable_real_to_logic={}
#     for i in range(index_train.__len__()):
#         lable_real_to_logic[index_train[i]]=i
#
#     net=resnet_ours.resnet32(num_classes=args.base_num_classes, device=device)
#     net = net.to(device)
#     num_old_classes=0
#
#     ave_acc = []
...
#
#
#             new_fc=resnet_ours.SplitCosineLinear(in_features,out_features,args.incre_num_classes).to(device)
...
#
#
#             new_fc=resnet_ours.SplitCosineLinear(in_features,out_features1+out_features2,args.incre_num_classes).to(device)
...
#             net = torch.load('./../checkpoints/ours/base_50_incre_10_task_0_model')
#             net = net.to(device)
#         else:
#             net.train()
#             if task_id>0:
...
#                         # feature_cross, output_cross = net.cross_forward(input, ref_net, w, features=True, all_attention=args.all_attention)
#                         # features_new, output=feature_cross, output_cross
#                         loss = loss_CE(output, label)
#                         train_loss_classification += loss
#
#
...
#                         loss_ours_freq=Compute_Pod_Space_Loss(features_new,features_old,device)*cur_lamda_pod
...
#                         evalset_pre = cifar.CIFAR100(root=args.dataset_dir, train=False, transform=transform_test,
#                                                      index=index_pre, target_transform=lable_real_to_logic)
#                         evalloader_pre = torch.utils.data.DataLoader(evalset_pre, batch_size=100, shuffle=True,
#                                                                      num_workers=2)
#                         # acc_pre = Compute.compute_accuracy_cross(net, ref_net, w, evalloader_pre, device)
...
#                     # acc_all = Compute.compute_accuracy_cross(net, ref_net, w, evalloader, device)
...
#
#         save_path = './../checkpoints/ours/base_{}_incre_{}_task_{}_model'.format(args.base_num_classes,args.incre_num_classes,task_id)
...
#         torch.save(class_means,'./../checkpoints/ours/class_means_task_{}'.format(task_id))
#
...
import numpy as np
import torch
import network.resnet_pod as resnet_ours
import torch.nn.functional as F
import torch.nn as nn
from tqdm import tqdm
from utils import Compute
...
def main():
    ...
    net=resnet_ours.resnet32(num_classes=args.base_num_classes, fc_num=args.fc_num, device=device)
    net = net.to(device)
    num_old_classes=0
