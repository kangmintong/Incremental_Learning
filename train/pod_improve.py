...
import numpy as np
import torch
import network.resnet_pod_improve as resnet_pod_improve
...
from utils.Compute_pod_improve import compute_accuracy
from utils.Compute_pod_improve import Compute_Feature_podnet
from utils.Compute_pod_improve import Compute_Pod_Space_Loss
from utils import net_copy
import math
from utils.Compute_pod_improve import Compute_Attention_Loss
...
def main():
    ...
    parser.add_argument('-pod_space_loss_weight', type=float, default=5.0)
    ...
    net=resnet_pod_improve.resnet32(num_classes=args.base_num_classes, fc_num=args.fc_num, device=device)
    net = net.to(device)
    num_old_classes=0
    ...
    for task_id in range(num_tasks+1):
        ...
        if task_id==1:
            ...
            for i in range(args.fc_num):
                new_fc=resnet_pod_improve.SplitCosineLinear(in_features,out_features,args.incre_num_classes).to(device)
            ...
            fc1s = []
            for i in range(args.fc_num):
                new_fc=resnet_pod_improve.SplitCosineLinear(in_features,out_features1+out_features2,args.incre_num_classes).to(device)
            ...
            for epoch in tqdm(range(args.epochs)):
                ...
                for (input,label) in trainloader:
                    ...
                    if task_id > 0:
                        ...
                        loss_pod_space = Compute_Attention_Loss(net, ref_net, output, ref_output, features_new, features_old, device) * cur_lamda_pod
                ...
                with torch.no_grad():
                    ...
                    acc_all = compute_accuracy(net, evalloader, device)
        ...
        with torch.no_grad():
            ...
            for previous_id in range(task_id+1):
                ...
                acc_pre=compute_accuracy(net,evalloader_pre,device)
