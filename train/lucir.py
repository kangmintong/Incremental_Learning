# normalized linear layer and feature
# marginranking loss to discriminate old and new scores
# divide fc with two parts corresponding the old task and the new task
# imprint weights: strategy to initialize new task parameters
# no optimizing on the parameters of the old task part of the fc
...
import numpy as np
import torch
import network.resnet_lucir as resnet_lucir
...
from utils.Compute import Compute_Feature_lucir
...
def main():
    ...
    net=resnet_lucir.resnet32(num_classes=args.base_num_classes)
    net = net.to(device)
    num_old_classes=0
    ...
    for task_id in range(num_tasks+1):
        ...
        if task_id==1:
            ...
            new_fc=resnet_lucir.SplitCosineLinear(in_features,out_features,args.incre_num_classes)
            new_fc.fc1.weight.data=net.fc.weight.data
            new_fc.sigma.data=net.fc.sigma.data
            net.fc=new_fc
            net=net.to(device)
            ...
            new_fc=resnet_lucir.SplitCosineLinear(in_features,out_features1+out_features2,args.incre_num_classes)
            new_fc.fc1.weight.data[:out_features1]=net.fc.fc1.weight.data
            new_fc.fc1.weight.data[out_features1:]=net.fc.fc2.weight.data
            new_fc.sigma.data=net.fc.sigma.data
            net.fc=new_fc
            net=net.to(device)
        ...
        if task_id>0:
            net=net_copy.imprint_weight(net,task_id,lable_real_to_logic,device,args)
            #args.epochs=60
        ...
        if args.resume_skip_base and task_id==0:
            net = torch.load('./../checkpoints/lucir/base_50_incre_10_task_0_model')
            net = net.to(device)
        else:
            net.train()
            if task_id>0:
            ...
            for epoch in tqdm(range(args.epochs)):
                ...
                for (input,label) in trainloader:
                    ...
                    optimizer.step()
                lr_scheduler.step()
                # print('epoch {} : loss_classification {} loss_kd {} loss_mr {} '.format(epoch,train_loss_classification,train_loss_kd,train_loss_mr))
        ...
        save_path = './../checkpoints/lucir/base_{}_incre_{}_task_{}_model'.format(args.base_num_classes,args.incre_num_classes,task_id)
        ...
        for i in range(0,args.base_num_classes+task_id*args.incre_num_classes,1):
            ...
            feature=Compute_Feature_lucir(loader,net,device,data.__len__())
